(function () {
    ...
    // ── Config ────────────────────────────────────────────────────────────────
    // Public feedback board via JSONBin (free, zero email, visible to all users).
    //
    // Setup (2 min):
    //   1. Go to https://jsonbin.io → sign up (free) → create a bin
    //   2. Set the bin content to: {"entries":[]}
    //   3. In bin settings, set Access to "Public" (so anyone can read)
    //   4. Copy the Bin ID and your Master Key below.
    //
    var JSONBIN_ID  = '';   // e.g. '664a1b2c3d4e5f6a7b8c9d0e'
    var JSONBIN_KEY = '';   // your Master Key from jsonbin.io → API Keys

    var COOLDOWN_MS = 60 * 60 * 1000;   // 1 hour between submissions per browser
    var LS_KEY      = 'ht_last_fb_submit';
    var MAX_DISPLAY = 20;               // show up to this many entries in the board
    ...
    // ── JSONBin helpers ───────────────────────────────────────────────────────
    var BIN_URL = 'https://api.jsonbin.io/v3/b/';

    function readBin() {
        return fetch(BIN_URL + JSONBIN_ID + '/latest', {
            headers: { 'X-Master-Key': JSONBIN_KEY },
        }).then(function (r) {
            if (!r.ok) throw new Error('read ' + r.status);
            return r.json();
        }).then(function (data) {
            return Array.isArray(data.record.entries) ? data.record.entries : [];
        });
    }

    function writeBin(entries) {
        return fetch(BIN_URL + JSONBIN_ID, {
            method:  'PUT',
            headers: {
    ...
    function initFeedback() {
        ...
        style.textContent = [
            ...
            '#ht-board{overflow-y:auto;max-height:14rem;',
        ...
        panel.innerHTML = [
            ...
            'padding:1rem 1.25rem 0.875rem;border-bottom:1px solid var(--border-color);">',
                '<div style="display:flex;align-items:center;gap:0.625rem;">',
                    ...
                    '</div>',
                    '<p id="ht-panel-title"',
                    ' style="font-size:0.875rem;font-weight:700;color:var(--text-main);margin:0;">',
                    'Suggestions & Feedback</p>',
                '</div>',
                '<button id="ht-feedback-x"',
                ' style="padding:0.25rem;border-radius:0.375rem;border:none;',
            ...
            // Board (submissions list)
            '<div id="ht-board" style="padding:0.75rem 1.25rem;">',
                '<p id="ht-board-loading"',
                ' style="font-size:0.8125rem;color:var(--text-muted);margin:0;',
                'text-align:center;padding:0.75rem 0;">Loading…</p>',
            '</div>',
            ...
            'border-top:1px solid var(--border-color);display:flex;flex-direction:column;gap:0.625rem;">',
                ...
                ' style="display:none;padding:0.45rem 0.625rem;border-radius:0.45rem;',
                ...
                ' style="display:none;font-size:0.75rem;color:#ef4444;margin:-0.125rem 0 0;">',
                'Please write your suggestion before submitting.</p>',

                '<p id="ht-submit-err"',
                ' style="display:none;font-size:0.75rem;color:#ef4444;margin:-0.125rem 0 0;"></p>',

                '<div style="display:flex;justify-content:flex-end;gap:0.5rem;">',
                    '<button id="ht-feedback-cancel"',
                    ' style="font-size:0.8125rem;padding:0.375rem 1rem;border-radius:0.375rem;',
                    'border:1px solid var(--border-color);background:transparent;',
                    'color:var(--text-muted);cursor:pointer;font-weight:500;">Cancel</button>',
                    '<button id="ht-send-btn"',
                    ' style="font-size:0.8125rem;padding:0.375rem 1.25rem;border-radius:0.375rem;',
    ...
    function loadBoard() {
        ...
        if (!JSONBIN_ID || !JSONBIN_KEY) {
            board.innerHTML = '<p style="font-size:0.8125rem;color:var(--text-muted);' +
                'margin:0;text-align:center;padding:0.75rem 0;">' +
                'Feedback board not configured yet.</p>';
            return;
        }

        board.innerHTML = '<p id="ht-board-loading" style="font-size:0.8125rem;' +
            'color:var(--text-muted);margin:0;text-align:center;padding:0.75rem 0;">Loading…</p>';

        readBin()
            .then(function (entries) { renderBoard(entries); })
            .catch(function () {
                board.innerHTML = '<p style="font-size:0.8125rem;color:var(--text-muted);' +
                    'margin:0;text-align:center;padding:0.75rem 0;">' +
                    'Could not load suggestions.</p>';
            });
    }
    ...
    function renderBoard(entries) {
        ...
        recent.forEach(function (entry, i) {
            ...
            var pagePart = entry.page ? entry.page + ' · ' : '';
            meta.textContent = pagePart + timeAgo(entry.ts);
    ...
    function sendFeedback() {
        ...
        if (!JSONBIN_ID || !JSONBIN_KEY) {
            serr.textContent = 'Feedback board not configured — add JSONBIN_ID and JSONBIN_KEY to shared.js.';
        ...
        var newEntry = { text: text, page: page, ts: new Date().toISOString() };

        readBin()
            .then(function (entries) {
                entries.push(newEntry);
                return writeBin(entries);
            })
            .then(function () {
    ...
    function showSuccessToast() {
        ...
        toast.innerHTML = '<svg style="width:1rem;height:1rem;flex-shrink:0" fill="none"' +
            ...
            '<span>Suggestion posted — thanks!</span>';
    ...
    function initUpdateChecker() {
        ...
        // On page load, strip any cache-bust param we added from a previous reload
        ...
        function showUpdateBanner() {
            ...
            document.getElementById('ht-reload-btn').addEventListener('click', function () {
                // Navigate with a cache-bust param so the browser fetches fresh HTML + JS.
                // localStorage (user settings, history) is untouched — only HTTP cache is bypassed.
